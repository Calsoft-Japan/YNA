codeunit 50001 GenJnlPostExt
{
    ...
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"CS_Gen. Jnl.-Post Batch", OnBeforeIfCheckBalance, '', false, false)]
    procedure CS2_OnBeforeIfCheckBalance(GenJnlTemplate: Record "Gen. Journal Template"; GenJnlLine: Record "Gen. Journal Line"; var LastDocType: Option; var LastDocNo: Code[20]; var LastDate: Date; var CheckIfBalance: Boolean; CommitIsSuppressed: Boolean; var IsHandled: Boolean)
    begin
        if GenJnlTemplate.Name in ['SALES', 'PURCHASES'] then begin
            IsHandled := true;
        end;
    end;

    //lewis  20250829 add GLRegno = Last GL: register No
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"CS_Gen. Jnl.-Post Batch", OnProcessLinesOnAfterPostGenJnlLines, '', false, false)]
    local procedure CS2_OnProcessLinesOnAfterPostGenJnlLines(var GenJournalLine: Record "Gen. Journal Line"; GLRegister: Record "G/L Register";
    ...
    procedure CS_OnBeforeIfCheckBalance(GenJnlTemplate: Record "Gen. Journal Template"; GenJnlLine: Record "Gen. Journal Line"; var LastDocType: Option; var LastDocNo: Code[20]; var LastDate: Date; var CheckIfBalance: Boolean; CommitIsSuppressed: Boolean; var IsHandled: Boolean)
    begin
        if GenJnlTemplate.Name in ['SALES', 'PURCHASES'] then begin
            IsHandled := true;
        end;
    ...
    //-------------------------------add lewis------------------------------------
    //CS 2025/1/10 Channing.Zhou FDD209
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"CSGen. Jnl.-Post", OnBeforeCode, '', false, false)]
    local procedure CSOnBeforeCode(var GenJournalLine: Record "Gen. Journal Line"; var HideDialog: Boolean)
    var
        APACC: Code[10];
        GLACC_CIP: Boolean;
        GLACCNON_CIP: Boolean;
    ...
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"CSGen. Jnl.-Post", OnCodeOnAfterGenJnlPostBatchRun, '', false, false)]
    local procedure CSOnCodeOnAfterGenJnlPostBatchRun(var GenJnlLine: Record "Gen. Journal Line")
    var
        applyentry: Codeunit VendEntryApplyPostEn;
        VendorLedEntry: Record "Vendor Ledger Entry";
        Custapplyentry: Codeunit CustEntryApplyPostEn;
